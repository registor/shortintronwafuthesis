...
% 包含beamer宏包
\documentclass[xcolor=svgnames, t, 10pt]{ctexbeamer}%fragile, 
...
{\LaTeX 科技排版简介}%\\[0.5ex] {\scriptsize V18.10}
...
\institute[
{\includegraphics[scale=0.01]{nwsuaflogo/nwsuaf_logo_cie}}\\ %插入学院
% logo
CS, CIE\\
NWSUAF\\
...
\begin{frame}{为什么要用\LaTeX?}{学位论文排版的需求}
  ...
  %   \begin{itemize}
  %   \item \nwafuthesis 模
  %     板：\url{https://github.com/registor/nwafuthesis}
  %   \end{itemize}
  \end{itemize}
  \stretchoff
\end{frame}
...
\begin{frame}{为什么要用\LaTeX?}{自由的召唤}
  \begin{columns}[c]
    ...
    \column{0.5\textwidth}
    \begin{center}
      \includegraphics[width=0.6\textwidth]{titlepage}\\
      \includegraphics[width=0.6\textwidth]{latexframe}\\%scale=0.05
      贴心\alert{秘书}
    \end{center}
  \end{columns}
\end{frame}
  ...
  \begin{center}
    \begin{minipage}{0.8\linewidth}
      ...
      \begin{block}{编译方案}
        ...
        成\verb|pdf|文件。\\
        如果是\alert{中文tex文档，优先使用{\XeLaTeX}}程序编译。
      \end{block}
    \end{minipage}
  \end{center}  
\end{frame}
...
\begin{frame}[t]{什么是\LaTeX?}{协作方便}
  ...
  \begin{center}
    \includegraphics[width=1.0\textwidth]{coworks}
  \end{center}
\end{columns}
\end{frame}
...
\begin{frame}[t]{什么是\LaTeX?}{PDF文件的批阅批注}
  ...
  \begin{center}
    \includegraphics[width=0.6\textwidth]{pdftakenotes}\\
    \includegraphics[width=0.6\textwidth]{todonotes}
  \end{center}
\end{columns}
% \stretchoff
\end{frame}
...
\begin{frame}[t]{安装\LaTeX 系统}{下载并启动安装}
  ...
  \end{itemize}
  \centering
  \includegraphics[height=.45\textheight]{texliveinst/fig02-00}
  \stretchoff
\end{frame}
\subsection[安装]{安装}
...
\begin{frame}[t]{安装\LaTeX 系统}{耐心等待}
    ...
    \end{itemize}
    \centering
    \includegraphics[width=.45\textwidth]{texliveinst/fig02-04}
    \hfill
    \includegraphics[width=.45\textwidth]{texliveinst/fig02-05}
    \\[2em]
    \includegraphics[width=.5\textwidth]{texliveinst/fig02-06}
\end{frame}
...
\begin{frame}[t,fragile]{安装\LaTeX 系统}{创建\tex 测试文档}
  ...
  \end{itemize}
  \begin{center}
    \includegraphics[width=.6\textwidth]{texliveinst/fig02-09}
  \end{center}
\end{frame}
...
\begin{frame}[t,fragile]{安装\LaTeX 系统}{编译\latex 源代码文件}
  \begin{itemize}
  \item 执行\verb|build.bat|编译{\LaTeX}源文件\verb|testLatex.tex|
  \item 得到\verb|testLatex.pdf|结果文件
  \end{itemize}
  \centering
...
\begin{frame}[t,fragile]{安装\LaTeX 系统}{查看编译结果}
  \begin{itemize}
  \item 双击\verb|testLatex.pdf|查看生成的到\verb|pdf|文件
  \end{itemize}
  \centering
  \includegraphics[width=.65\textwidth]{texliveinst/fig02-13}
  \begin{block}{结果正确}
    到此，表示{\TeX}Live安装正常，能够正常工作，配置成功。
  \end{block}
\end{frame}

\begin{frame}[t, fragile]{安装\LaTeX 系统}{网络安装方式}
    \begin{itemize}
    \item 通过网络在线安装与上述过程类似，可参考
      \url{http://www.tug.org/texlive/}的指导进行安装
    \item 安装结束后，也可以按上述方式进行测试
    \end{itemize}
    \begin{center}
      \includegraphics[width=.9\textwidth]{texliveinst/fig02-000}
    \end{center}
\end{frame}

\subsection[IDE]{IDE环境}
\begin{frame}[t, fragile]{安装\LaTeX 系统}{安装编辑器}
  \begin{itemize}  
  \item 可以使用任何一个编辑器做为\LaTeX 的IDE:    
  \end{itemize}
  \begin{center}
    \includegraphics[height=.3\textheight]{texliveinst/fig02-22}
    \hfill
    \includegraphics[height=.3\textheight]{texliveinst/texstudio}
    \\[2em]
    \includegraphics[width=.4\textwidth]{texliveinst/WinEdit}
  \end{center}  
\end{frame}
%%%%%%%%%%%%%%%%
...
\begin{frame}[t,fragile]{学习\LaTeX}{基本约定}
  ...
  \item 换行：1个回车仅视为1个空格，2个以上回车视为1个换行
  \end{itemize}
  \stretchoff
\end{frame}
