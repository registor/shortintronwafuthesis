...
\begin{itemize}
  \item 打开终端
    ...
    \begin{itemize}
      \item \faWindows{}：右键开始菜单、空白处 \kbd{Shift} + 右键、\kbd{Windows} + \kbd{R} \& \texinline{cmd}
...
\begin{frame}[t]{\nwafuthesis 模板}{模板使用}
  ...
  %   \item $\ldots$
  %   \end{itemize}
  \item 用\texinline{\documentclass{nwafuthesis}}引入文档类(模板)\footnote[frame]{看文档、看文档、看文档}
    \begin{itemize}
    \item 用可选参数指定论文类型
    \end{itemize}
  \item 用\texinline{\nwafuset}和\texinline{\nwafusetEn}命令设置论文基本
...
\begin{frame}{\nwafuthesis 模板}{模板使用}
  ...
  \item 文献
    ...
    \item \pkg{biber} 后端 + \pkg{biblatex} 宏包

      \begin{itemize}
      \item 模板已设置，\alert{无需更改}
      \end{itemize} 

    \item 需多次编译---强烈推荐 \pkg{latexmk}
    \end{itemize}
  \end{itemize}
  \stretchoff
\end{frame}
